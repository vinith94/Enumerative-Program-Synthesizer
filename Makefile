...
SYNTH_LIB_PARSER_TESTER=$(SYNTH_LIB_PARSER_ROOT)/bin/$(BUILD_SUFFIX)/Solver
...
ifeq "x$(VERBOSE_BUILD)" "x"
		@echo "$(LDPRINTNAME) `basename $@`"; \
		$(CXX) $(CXXFLAGS) $(OPTFLAGS) $(SYNTH_LIB_PARSER_TESTER_OBJS) -lz3 -L $(SYNTH_LIB_PARSER_ROOT)/lib/$(BUILD_SUFFIX) -Wl,-Bstatic -lsynthlib2parser -Wl,-Bdynamic -o $@
else
		$(CXX) $(CXXFLAGS) $(OPTFLAGS) $(SYNTH_LIB_PARSER_TESTER_OBJS) -lz3 -L $(SYNTH_LIB_PARSER_ROOT)/lib/$(BUILD_SUFFIX) -Wl,-Bstatic -lsynthlib2parser -Wl,-Bdynamic -o $@
endif
